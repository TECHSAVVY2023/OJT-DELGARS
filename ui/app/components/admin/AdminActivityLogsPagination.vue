    ...
    <div class="flex items-center gap-4">
      <div class="flex items-center gap-2">
        ...
        <select
          ...
          @change="(e: Event) => emit('update:rowsPerPage', Number((e.target as HTMLSelectElement).value))"
      ...
      <span class="text-sm text-gray-600">
        Showing {{ (props.currentPage - 1) * props.rowsPerPage + 1 }}–{{ Math.min(props.currentPage * props.rowsPerPage, props.totalPages * props.rowsPerPage) }} entries
      </span>
    </div>

    <div class="flex items-center gap-2">
      <button
        @click="() => emit('update:currentPage', Math.max(1, props.currentPage - 1))"
      ...
      <div class="flex items-center gap-1">
        <button
          v-for="page in props.visiblePageNumbers"
          :key="page"
        ...
        <button
          v-if="props.totalPages > 5 && !props.visiblePageNumbers.includes(props.totalPages)"
      ...
      <button
        @click="() => emit('update:currentPage', Math.min(props.totalPages, props.currentPage + 1))"
