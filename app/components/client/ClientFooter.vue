  ...
  <footer class="bg-[#8B0101] text-white py-12">
    <div class="max-w-[96rem] mx-auto px-3 sm:px-4 lg:px-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 pb-8 border-b border-red-700">
        <!-- Contact -->
        <div>
          <h3 class="text-lg font-bold mb-4">{{ footer.reachTitle }}</h3>
          <div class="space-y-2 text-gray-200">
            <p class="flex items-center gap-2">
              <Icon name="mdi:phone" class="w-5 h-5 text-gray-200" />
              {{ footer.phone }}
            </p>
            <p>{{ footer.email }}</p>
            <p class="text-sm">
        ...
        <!-- Company -->
        <div>
          <h3 class="text-lg font-bold mb-4">{{ footer.companyTitle }}</h3>
        ...
        <div class="bg-gray-900 p-4 rounded-lg">
          <h3 class="text-lg font-bold mb-3">{{ footer.newsletterTitle }}</h3>
          <input
            type="email"
            :placeholder="footer.newsletterPlaceholder"
            class="w-full px-3 py-2 bg-gray-800 rounded text-white placeholder-gray-500 mb-3 outline-none focus:bg-gray-700 transition"
          />
          <button
            type="button"
            class="w-full py-2 bg-blue-500 hover:bg-blue-600 transition rounded font-semibold"
