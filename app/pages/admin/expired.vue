...
import { useExpiredProductsData } from "~/composables/useExpiredProductsData";

const route = useRoute();
const { success, error, info } = useToast();
const { expiredProducts } = useExpiredProductsData();
...
const handleEditProduct = (productId: string) => {
  success(`Editing product: ${productId}`);
};
  ...
  <div class="flex h-screen bg-gray-50" style="font-family: 'Nunito Sans', sans-serif">
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      ...
      <div class="flex-1 flex flex-col overflow-hidden bg-gray-50">
          ...
          <div class="flex items-center justify-between mb-4">
            <div>
              <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
                ...
                <span class="text-gray-900 font-medium">Expired Products</span>
              </div>
              <h1 class="text-3xl font-bold text-gray-900">Expired Products</h1>
            </div>
            <div class="flex items-center gap-2">
              <button class="p-2.5 hover:bg-gray-100 rounded-lg transition" title="Export to PDF">
                <Icon name="mdi:file-pdf" class="w-7 h-7 text-red-500" />
              </button>
              <button class="p-2.5 hover:bg-gray-100 rounded-lg transition" title="Export to Excel">
                <Icon name="mdi:file-excel" class="w-7 h-7 text-green-600" />
              </button>
              <button class="p-2.5 hover:bg-gray-100 rounded-lg transition" title="Print">
                <Icon name="mdi:printer" class="w-7 h-7 text-gray-600" />
              </button>
              <button class="p-2.5 hover:bg-gray-100 rounded-lg transition" title="Refresh">
                <Icon name="mdi:refresh" class="w-7 h-7 text-gray-600" />
              </button>
            </div>
          </div>
        ...
        <!-- Table (same container/padding as products page) -->
            ...
            <table class="w-full border-collapse">
              <thead>
                <tr class="border-b border-gray-200 bg-gray-50">
                  ...
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Manufactured Date ⬇</th>
                  <th class="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Expired Date ⬇</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-6 py-4 text-sm text-gray-600 text-center">{{ product.manufacturedDate }}</td>
                  <td class="px-6 py-4 text-sm text-gray-600 text-center">{{ product.expiredDate }}</td>
                  <td class="px-6 py-4 text-center">
